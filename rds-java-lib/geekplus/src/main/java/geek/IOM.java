...
IOM
  ...
  cycle()
    ...
    List<Map<String,String>> requests 
      = db.getResultMapList("SELECT * FROM gmsOperation "
                           +"WHERE processed='no' ") ;
    if((requests==null) || (requests.size()==0))
      return ;

    int total = requests.size() ;
    RDSLog.inform("iom: unprocessed %d",total) ;
    ...
    for(Map<String,String> request : requests)
      ...
      if( (requestId==null) || (requestId.length()==0))
        ...
        db.execute("UPDATE gmsOperation "
                  +"SET requestId='%s' "
      ...
      body.put("instruction",request.get("instruction")) ;
      body.put("waitLocation",request.get("waitLocation")) ;
      ...
      String url = "http://"
                 ...
                 + ":"
                 + port 
                 + "/geekplus/api/artemis/pushJson/iom" ;
      ...
      try
        ...
        if(result_code==200)
          ...
          RDSLog.trace("iom: code %d",code) ;
          if(code==0)
            {
            success=true ;

            GeekUpdate u1 = new GeekUpdate(db,"gmsOperation",result_header) ;
            ...
            GeekUpdate u2 = new GeekUpdate(db,"gmsOperation",result_body) ; 
            u2.addKeyIntLiteral(seq,"seq") ;
            u2.addString("workflowCode") ;
            u2.addString("taskCode") ;
            u2.addString("workFlowCode") ;
          ...
          RDSLog.alert("iom: http call fails, code %d",result_code) ;
        }
      catch(Exception e) { e.printStackTrace() ; }
