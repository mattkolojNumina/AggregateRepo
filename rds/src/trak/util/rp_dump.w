...
% --- title block ---
\def\title{Register Point Dump Utility}
\def\topofcontents{\null\smallskip
...
@* Overview. 
A utility to dump the values of Trak RPs.
...
@ Dump function.
@<Functions@>+=
int each_rp(int handle)
   {
   char name[TRAK_NAME_LEN+1], desc[TRAK_DATA_LEN+1], 
        drv[TRAK_DATA_LEN+1], drvdesc[TRAK_DATA_LEN+1] ;
   int value ;

   rp_settings(handle,name,desc,drv,drvdesc) ;
   value = rp_get(handle) ;
   printf("%s\t%d\n",name,value) ;
   }
@* Index.
