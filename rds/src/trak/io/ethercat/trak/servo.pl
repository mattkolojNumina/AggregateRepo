...
# ----- r e g i s t e r s -----
#
($SERVO_REG_STATE, $SERVO_REG_TIMER, $SERVO_REG_DEBUG,
 $SERVO_REG_FAULT, $SERVO_REG_RESET,
 $SERVO_REG_READY, $SERVO_REG_MOVING,
 $SERVO_REG_SPEED, $SERVO_REG_POSITION, $SERVO_REG_ACCEL,
...
# write value of ACCEL rp to 7024 (decel) 
#
# if POSITION rp != 7022 (position)
#  write POSITION rp to 7022
...
@code=qw(
  ...
  SERVO_REG_POSITION r@ dp.register.get rp.value.get
  SERVO_REG_7022     r@ dp.register.get s@ = not
  if
    SERVO_REG_POSITION r@ dp.register.get rp.value.get
...
createServo
  ...
  $rp = rp::const($name.'_position',0,$name.' position') ;
  Trk::dp_registerset($dp,$SERVO_REG_POSITION,$rp) ;
