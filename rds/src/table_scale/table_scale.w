%
%   table_scale.w -- process a device message associated with a trak zone
%
...
\centerline{Control Date: $ $Date: 2023/10/04 14:11:09 $ $}
}
...
@c
static char rcsid[] = "$Id: table_scale.w,v 1.9 2023/10/04 14:11:09 rds Exp rds $";
@<Includes@>@;
@<Defines@>@;
@<Prototypes@>@;
@<Globals@>@;
...
void handle_msg( const char msg[] ) {
   ...
   Inform( "processing %s, took %.3f sec",
          "successful" , util_get_elapsed( start_time ) );
}
@ Prototype the function.
@<Prototypes@>+=
void handle_msg( const char msg[] );
