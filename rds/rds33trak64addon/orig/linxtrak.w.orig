...
\centerline{RCS ID: $ $Id: linxtrak.w,v 1.39 2019/02/25 20:34:23 rds Exp $ $}
}
...
@c
static char rcsid[] = "$Id: linxtrak.w,v 1.39 2019/02/25 20:34:23 rds Exp $" ;
@<Defines@>@;
@<Includes@>@;
@<Structures@>@;
@<Globals@>@;
...
linx_init(void) 
  ...
  if (!(pid=fork())) 
    ...
    server_pid = pid ;
    Inform("fork server pid %d",pid) ;

    for (i=0 ; i < N_CARDS ; i++) 
      { 
      // for each card defined
      idlinx_struct *mycard = cards + i ;
      if (mycard->octet[0]==0) 
        continue ;
      Inform("fork for card %d",i) ;
      if (!(pid=fork())) 
        {
        char procname[32+1] ;
        sprintf(procname,"linx_%d",i) ;
        trn_register(procname) ;
        @<process one card@>@;
        } 
      else 
        {
        mycard->pid = pid ;
        Inform("fork pid %d",pid) ;
        continue ;
        }
      } 
    }
...
linx_stop(int term)
  ...
  for(n=0 ; n<N_CARDS ; n++)
    {
    idlinx_struct *card = cards + n ;
    if(card->pid)
      kill(card->pid,SIGTERM) ;
    }  
...
@<UDP server (handles all inputs)@>=
  ...
  Trace("udp server bound to socket 2222") ;
  ...
  for (;;) 
    ...
    int i ;
    len = sizeof( struct sockaddr_in );
    err = recvfrom( s, request, 200, 0, ( struct sockaddr* )&remote, &len);
    if (err < 0) 
      continue ;
...
@<register@>=
  ...
  s = net_open(mycard->ip,ETHERNETIP_PORT) ;
  if (s < 0) 
    {
  ...
  optval = 1 ;
  setsockopt(s,SOL_SOCKET,SO_KEEPALIVE,&optval,len) ;
  ...
  Inform("opened sockets TCP: %d UDP: %d",s,us) ;
  m = 0 ;
...
@<send forward packet@>=
  ...
  for (i=0 ;i < 4 ; i++) 
    out[c++] = mycard->session[i] ;
  out[c++] = 0xD8 ; // connection serial number 
  out[c++] = 0xA5 ; 
