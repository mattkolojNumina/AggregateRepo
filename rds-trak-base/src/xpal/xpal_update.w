%
%   xpal_update.w -- update carton data
%
...
@<get carton@>+=
  ...
  if(barcode_success)
    {
    if(!util_required(seq,"carton")) 
      {
      Inform( "%03d: carton not required for [%d]", box, seq );
      strcpy(description,"carton not required") ;
      }
    else
      {
      ...
      if(cartonSeq>0)
        {
        Inform("%03d: %d: found carton %d",box,seq,cartonSeq) ;
        strcpy(description,"carton found") ;
        util_box_set( box, "cartonSeq", string(cartonSeq) );
        util_carton_set( area, seq, "cartonSeq", string(cartonSeq) );
        util_update_status( seq, "carton", "complete",string(cartonSeq));
        }
      else
        {
        Inform("%03d: %d: carton not found",box,seq) ;
        strcpy(description,"carton not found") ;
        util_update_status(seq,"carton","failed","") ;
        }
      util_update_description( area, seq, NULL, NULL, description );
      }
    }
...
@<check ready@>+=
  {
  if( (cartonSeq>0) && app_carton_ready(cartonSeq) )
    util_update_status(seq,"ready","complete","") ;
  else
    {
    char prn1[BUF_LRG+1] ;
    char prn2[BUF_LRG+1] ;

    util_update_status(seq,"ready","failed","not ready to label") ;

    snprintf(prn1,BUF_LRG,"%sPrn1",area) ;
    prn1[BUF_LRG] = '\0' ;
    util_update_status(seq,prn1,"optional","") ;

    snprintf(prn2,BUF_LRG,"%sPrn2",area) ;
    prn2[BUF_LRG] = '\0' ;
    util_update_status(seq,prn2,"optional","") ;
    }

  if( (cartonSeq>0) && app_carton_needs_qc(cartonSeq) )
    util_update_status(seq,"qc","failed","schedule to qc") ;
  else
    util_update_status(seq,"qc","optional","") ;
