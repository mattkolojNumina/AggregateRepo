...
%      10/24/22 -- added new hist_post for 'log' table
%
%
...
\centerline{RCS ID: $ $Id: rds_hist.w,v 1.6 2024/03/26 18:37:31 rds Exp rds $ $}
}
...
int hist_post(const char *carton, const char *code, const char *description)
   {
   return log_post(carton, "carton", code, description);
   }
...
is called by applications to post a carton history message.
@<Exported Functions@>+=
int log_post(const char *id, const char *type, const char *code, const char *description)
   {
   ...
   snprintf(msg,HIST_MSG_LEN,"%s\t%s\t%s\t%ld\t%ld\t%s",
         id,type,code,tv.tv_sec,tv.tv_usec,description) ;
   msg[HIST_MSG_LEN] = '\0' ;
