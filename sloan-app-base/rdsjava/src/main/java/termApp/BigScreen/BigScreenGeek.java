...
extends AbstractBigScreen {
   ...
   public BigScreenGeek(TerminalDriver term) {
      super(term);
   }
...
//-----------------------------------DRAWS CARTON COUNTS LIST-----------------------------------
   ...
   public void initDisplay() {
      ...
      helloText = new TextField(SCREEN_WIDTH/2, 0, 75, "Geek +", Align.CENTER);
...
//         inform("numWaves = [%d], remainingWaves = [%d]", numWaves, remainingWaves);
         while (drawWave == false) {
            bkgdColor = getBackgroundColorGeek(cartonList, i, PICKTYPE_GEEK);
