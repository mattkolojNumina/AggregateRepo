...
extends AbstractBigScreen {
   ...
   public BigScreenDU(TerminalDriver term) {

      super(term);
      cartonList = getBuildListDU();
   }
...
//-----------------------------------DRAWS CARTON COUNTS LIST-----------------------------------
   ...
   public void initDisplay() {
      ...
      helloText = new TextField(SCREEN_WIDTH/2, 0, 75, "BoomRoom", Align.CENTER);
...
//         inform("numWaves = [%d], remainingWaves = [%d]", numWaves, remainingWaves);
         while (drawWave == false) {
            bkgdColor = getBackgroundColor(cartonList, i, PICKTYPE_AERSOLBOOM);
