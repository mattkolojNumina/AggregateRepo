...
public class VerifyLabelScreen extends AbstractOverPackCartonLabelStationScreen {

   private Button ok,cancel;
   private List<printObj> printSeqList;
   private boolean checkPrintJob;

   public VerifyLabelScreen( TerminalDriver term ) {
      super(term) ;
   ...
   private void doPrint(int docSeq) {
      processing = 0;
      int printJobSeq = printRdsDocument(docSeq,getStrParam("zebraPrinter"));
      if( printJobSeq > 0 ) {
      	printObj o = new printObj(docSeq,printJobSeq);
         printSeqList.add(o);
         trace( "printing label docSeq %d, jobSeq %d",o.docSeq, o.printJobSeq);
         checkPrintJob = true;
   ...
   public void inputDecision(String scan) {
      ...
      showAlertMsg("No label found with [%s]",scan);
   }
   ...
   private void checkPrintJob() {
      ...
      if( checkPrintJob && processing > 0 && System.currentTimeMillis() - processing > timeout ) {
         showAlertMsg("Reprinting timeout, check Zebra printer.");
         checkPrintJob = false;
         processing = 0;
      }
   }
