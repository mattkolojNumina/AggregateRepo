...
public class ValidateCartonScreen extends AbstractOverPackCartonLabelStationScreen {
      ...
      if( validateMode == VALIDATE_MODE_LPN ) {
      	...
      	if( !isValidSkuBarcode(scan,validateValue,validateUom) )
      		...
      		if( requirePacklist() ) {
      			...
      			nextScreen = "overPackCartonLabelStation.CheckPacklistScreen";
      			ok.show();
      			initStartTimer();  
      			...
      			nextScreen = "overPackCartonLabelStation.VerifyLabelScreen";
      			ok.show();
      			initStartTimer();      			
      		}
      	}      	
