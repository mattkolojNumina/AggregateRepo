...
public class PickTranslator extends AbstractDAO implements GeekTranslator {
    ...
    private void pickOrderConfirmation()  {
        ...
        try {
            ...
            allUnprocessedPickingConfirmations = geekDAO.getAllProcessedOrdered("geekPickConf", "no", "seq", "ASC");
            ...
            if(allUnprocessedPickingConfirmations != null && !allUnprocessedPickingConfirmations.isEmpty()){
                ...
                for (Map<String, String> unprocessedPickingConf : allUnprocessedPickingConfirmations) {
                    ...
                    trace("processing geekPickConfSeq [%d]",geekPickOrderConfSeq);
                    /**
                     * rds level changes
                     */
                    Map<String,String> recordMap;
                    out_order_code = unprocessedPickingConf.get("out_order_code");
                    recordMap = getTableRowByStringId("rdsCartons", "trackingNumber", out_order_code);
                    if(recordMap == null || !recordMap.containsKey("cartonSeq")){
                        alert("unknown rdsCartons.trackingNumber ~ (geekPickConf.out_order_code) [%s], skipping the row",out_order_code);
                        int rowsUpdated = geekDAO.updateProcessed("geekPickConf","err",geekPickOrderConfSeq);
                        if(rowsUpdated != 1){
                            alert("error while setting geekPickConf.seq[%d] as processed = 'err'",geekPickOrderConfSeq);
                        }
                        continue;
                    }
                    ...
                    if(recordMap.get("pickStamp") != null ){
                        alert("rdsCartons.trackingNumber ~ (geekPickConf.out_order_code) [%s] is already picked, skipping the row",out_order_code);
                        int rowsUpdated = geekDAO.updateProcessed("geekPickConf","yes",geekPickOrderConfSeq);
                        if(rowsUpdated != 1){
                            alert("error while setting geekPickConf.seq[%d] as processed = 'yes'",geekPickOrderConfSeq);
                        }
                        continue;
                    }
                    if(recordMap.get("cancelStamp") != null){
                        alert("rdsCartons.trackingNumber ~ (geekPickConf.out_order_code) [%s] is canceled, skipping the row",out_order_code);
                        int rowsUpdated = geekDAO.updateProcessed("geekPickConf","yes",geekPickOrderConfSeq);
                        if(rowsUpdated != 1){
                            alert("error while setting geekPickConf.seq[%d] as processed = 'yes'",geekPickOrderConfSeq);
                        }
                    ...
                    trace("processing geekPickConfSeq [%d], trackingNumber[%s], cartonSeq[%d]",geekPickOrderConfSeq,out_order_code,rdsCartonsSeq);
                    ...
                    List<Map<String, String>> geekPickConfContainerSeqList = geekDAO.getAllByParent(geekPickOrderConfSeq,"geekPickConfContainer");
                    geekPickConfContainerSeqListSize = geekPickConfContainerSeqList.size();
                    trace("for pickConf[%d], [%d] rows found in geekPickConfContainer ", geekPickOrderConfSeq, geekPickConfContainerSeqListSize);

                    // ask tim if this is 1:1 or can one pick conf can have more than one container
                    // as of now ecommerce orders will have more than one LPNs which is 5% of their orders
                    for(Map<String, String> geekPickConfContainerSeq : geekPickConfContainerSeqList){
                        container_code = geekPickConfContainerSeq.get("container_code");
                        updateRowByIntIdStringValue("rdsCartons","lpn",container_code,"cartonSeq",rdsCartonsSeq);
                    }

                    List<Map<String, String>> geekPickConfSkuList = geekDAO.getAllByParent(geekPickOrderConfSeq,"geekPickConfSku");
                    confSkusListSize = geekPickConfSkuList.size();
                    trace("for pickConf[%d], [%d] rows found in geekPickConfSku", geekPickOrderConfSeq, confSkusListSize);
                    int pickup_amount = 0;
                    int plan_amount = 0;
                    ...
                    for(Map<String, String> geekPickConfSku : geekPickConfSkuList){
                        ...
                        trace("processing pick conf seq  : [%d]-[%d]", geekPickOrderConfSeq, geekPickOrderConfSkuSeq);

                        rdsPicksSeq = Integer.parseInt(geekPickConfSku.get("out_batch_code"));
                        part_number = geekPickConfSku.get("sku_code");
                        quantity_change = Integer.parseInt(geekPickConfSku.get("pickup_amount"));
                        pickup_amount = getMapInt(geekPickConfSku,"pickup_amount");
                        plan_amount = getMapInt(geekPickConfSku,"plan_amount");

                        trace("         container_code      : [%s]", container_code);
                        ...
                        trace("         pickup_amount       : [%d]", pickup_amount);
                        trace("         planned_amount      : [%d]", plan_amount);
                         ...
                         */
                        if(true){ // if customer does not want adjustments when nothing is picked i.e. pickup_amount = 0, then add that condition in this IF condition i.e. && quantity_change > 0
