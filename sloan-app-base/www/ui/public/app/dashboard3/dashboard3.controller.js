(function()
{
  angular
    .module('ui')
      .controller('Dashboard3Controller',dashboard3Controller);

  angular
    .module('ui')
      .config(dashboard3Config);

  dashboard3Controller.$inject = ['$scope','$timeout','$routeParams',
                              '$mdDialog','$interval','Global','DbFactory'];
  
  function dashboard3Controller($scope,$timeout,$routeParams,$mdDialog,$interval,Global,DbFactory) {
    
    var periodic;

    $scope.refresh = refresh;
    $scope.permit = Global.permit;

    $scope.today = {};
    $scope.geek = {};
    $scope.zoneRoute = {};
    $scope.duCarts = {};
    $scope.pqCarts = {};
    $scope.selectedDate = new Date();
    $scope.dailyWaveSeq = "All";
    $scope.waveSeqs = [];
    $scope.dzr = $scope.dzr || {};
    $scope.shorts = {};
	
    // // // // //
    // TABLE HELPERS  
    var refreshCounter = 0;
    
    function refreshCount(n,name) {
      refreshCounter += n;
      if(refreshCounter>0)
        Global.busyRefresh(true);
      else
        Global.busyRefresh(false);
      if(false) //make true for logging
        console.log(name+": "+refreshCounter);
    } 

    function executeError(err) {
      console.log(err);
      refreshCount(-1, "error");		
    }

    function executeErrorToday(err) {
      console.log(err);
      refreshCount(-1, "error");		
    }

    function executeErrorGeek(err) {
      console.log(err);
      refreshCount(-1, "error");		
    }

    function executeErrorZR(err) {
      console.log(err);
      refreshCount(-1, "error");		
    }

    function executeErrorDU(err) {
      console.log(err);
      refreshCount(-1, "error");		
    }

    function executeErrorPQ(err) {
      console.log(err);
      refreshCount(-1, "error");		
    }

    function executeErrorShorts(err) {
      console.log(err);
      refreshCount(-1, "error");		
    }

    function toDueDate(date) {
      return date.getFullYear() + "-" +
      ('00' + (date.getMonth() + 1)).slice(-2) + "-" + 
      ('00' + (date.getDate())).slice(-2)
    }

    // // // // //
    // DATA RETRIEVAL
    //======================================================================================================================================
    // // //
    // TAB 0 Demand Today
    
    function refreshDemandToday() {
      //console.log("Demand date: " + $scope.selectedDate)
      $scope.formattedDate = toDueDate(new Date($scope.selectedDate))
      //console.log("Formatted date: " + $scope.formattedDate)

      DbFactory.post({topic: 'dashboard3',
                      action: 'getWaves',
                      params: {
                        formattedDate: $scope.formattedDate
                      }
                    })
        .success(setWaves)
        .error  (executeError);

      DbFactory.post({topic: 'dashboard3',
                      action: 'today',
                      params: {dailySeq: $scope.dailyWaveSeq,
                        formattedDate: $scope.formattedDate
                      }
                     })
        .success(demandTodaySuccess)
        .error  (executeErrorToday);

      DbFactory.post({topic: 'dashboard3',
                      action: 'geek',
                      params: {dailySeq: $scope.dailyWaveSeq,
                        formattedDate: $scope.formattedDate
                      }
                    })
        .success(demandGeekSuccess)
        .error  (executeErrorGeek);

      DbFactory.post({topic: 'dashboard3',
                        action: 'zoneRouteToday',
                        params: {dailySeq: $scope.dailyWaveSeq,
                          formattedDate: $scope.formattedDate
                        }
                      })
        .success(demandTodayZoneRouteSuccess)
        .error  (executeErrorZR);

      DbFactory.post({topic: 'dashboard3',
                        action: 'duCarts',
                        params: {dailySeq: $scope.dailyWaveSeq,
                          formattedDate: $scope.formattedDate
                        }
                      })
        .success(demandDUCartsSuccess)
        .error  (executeErrorDU);

      DbFactory.post({topic: 'dashboard3',
                        action: 'pqCarts',
                        params: {dailySeq: $scope.dailyWaveSeq,
                          formattedDate: $scope.formattedDate
                        }
                      })
        .success(demandPQCartsSuccess)
        .error  (executeErrorPQ);

      DbFactory.post({topic: 'dashboard2',
                        action: 'shortPicks',
                        params: {dailySeq: $scope.dailyWaveSeq}
                      })
        .success(demandShortsSuccess)
        .error  (executeErrorShorts);
    }

    function setWaves(data) {
      var waves = [];
      for(var i=0; i<data.length;i++) {
        waves.push(data[i].dailyWaveSeq);
      }
      $scope.waveSeqs = waves;
    }
    
    function demandTodaySuccess(data) {
        $scope.today = data[0];
        //console.log("scope.formattedDate: " + $scope.formattedDate)
        //console.log("scope.dailyWaveSeq: " + $scope.dailyWaveSeq)
        //console.log("scope.today: " + $scope.today)
        //console.log("scope.today.numOrdersDownloaded: " + $scope.today.numOrdersDownloaded)
    }

    function demandShortsSuccess(data) {
      if($scope.dailyWaveSeq==='All') {
        $scope.shorts = data[0];
      }
      else {
        $scope.shorts = data[1];
      }
    }

    function demandGeekSuccess(data) {
        $scope.geek = data[0];
    }

    function demandTodayZoneRouteSuccess(data) {
        $scope.zoneRoute = data[0];
    }

    function demandDUCartsSuccess(data) {
        $scope.duCarts = data[0];
    }

    function demandPQCartsSuccess(data) {
        $scope.pqCarts = data[0];
        $scope.pqCarts.numPQCartons = data[0].numLiquidCartons + data[0].numPerishableCartons;
    }
    
    //======================================================================================================================================
    // // //
    // TAB 1 Demand Zone Route

    function refreshDemandZoneRoute() {
      //console.log("Demand date: " + $scope.selectedDate)
      $scope.formattedDate = toDueDate(new Date($scope.selectedDate))
      //console.log("Formatted date: " + $scope.formattedDate)

      DbFactory.post({topic: 'dashboard3',
        action: 'getWaves',
        params: {
          formattedDate: $scope.formattedDate
        }
      })
      .success(setWaves)
      .error  (executeError);

      if($scope.dailyWaveSeq==='All') {
      DbFactory.post({topic: 'dashboard3',
                      action: 'zr',
                      params: {
                        formattedDate: $scope.formattedDate
                      }
                     })
        .success(demandZoneRouteSuccess)
        .error  (executeErrorZRTab);
      }
      else {
        DbFactory.post({topic: 'dashboard3',
                        action: 'zrByWave',
                        params: {dailySeq: $scope.dailyWaveSeq,
                          formattedDate: $scope.formattedDate
                        }
                      })
        .success(demandZoneRouteSuccess)
        .error  (executeErrorZRTab);
      }
    }

    function executeErrorZRTab(err) {
      console.log(err);
      refreshCount(-1, "error");		
    }

    function demandZoneRouteSuccess(data) {
      $scope.dzr = [];
      $scope.dzr[0] = {0:0};
      $scope.dzr = $scope.dzr.concat(data);
    }
    
    // // // // //
    // SETUP AND ALL THAT

    function changeDate() {
      $scope.dailyWaveSeq = "All";
      refresh();
    }

    function
    refresh()
    {
      switch($scope.selected){
        case 0:
          refreshDemandToday();
          break;				
        case 1:
          refreshDemandZoneRoute();
          break;					
        default:
          refreshCount(0, "default tab rendered");
      }
    }

    function
    init()
    {
      Global.setTitle('Dashboard');
      Global.recv('refresh',refresh,$scope);
      Global.recv('changeDate',changeDate,$scope);
      periodic = $interval(refresh, 30000); 
      refresh();
    }

    init();

    $scope.$on('$destroy', function(){
      $interval.cancel(periodic);
    });
    
  }

  function
  dashboard3Config($routeProvider)
  {
    $routeProvider
      .when('/dashboard3', {controller: 'Dashboard3Controller',
                        templateUrl: '/app/dashboard3/dashboard3.view.html'});
  }
  
}())

